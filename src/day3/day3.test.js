...
describe("day 3", () => {
  test("move left", () => {
    var wires = createWires("L5");
    process(origin, wires);
    expect(wires[0].positions.length).toBe(6);
    var finalLoc = wires[0].positions[wires[0].positions.length - 1];
    expect(finalLoc.x).toBe(-5);
    expect(finalLoc.y).toBe(0);
  });

  test("move right", () => {
    var wires = createWires("R5");
    process(origin, wires);
    expect(wires[0].positions.length).toBe(6);
    var finalLoc = wires[0].positions[wires[0].positions.length - 1];
    expect(finalLoc.x).toBe(5);
  ...
  test("move up", () => {
    var wires = createWires("U5");
    process(origin, wires);
    expect(wires[0].positions.length).toBe(6);
    var finalLoc = wires[0].positions[wires[0].positions.length - 1];
    expect(finalLoc.x).toBe(0);
    expect(finalLoc.y).toBe(-5);
  });

  test("move down", () => {
    var wires = createWires("D5");
    process(origin, wires);
    expect(wires[0].positions.length).toBe(6);
    var finalLoc = wires[0].positions[wires[0].positions.length - 1];
    expect(finalLoc.x).toBe(0);
  ...
  test("move square", () => {
    var wires = createWires("R5,U5,L5,D5");
    process(origin, wires);
    expect(wires[0].positions.length).toBe(21);
    var finalLoc = wires[0].positions[wires[0].positions.length - 1];
    expect(finalLoc.x).toBe(0);
    expect(finalLoc.y).toBe(0);
  });

  test("self crossing wire", () => {
    var wires = createWires("R10,U5,L5,D10,L5,U5");
    process(origin, wires);
    output(wires);
    expect(wires[0].positions.length).toBe(41);
    var finalLoc = wires[0].positions[wires[0].positions.length - 1];
    expect(finalLoc.x).toBe(0);
    expect(finalLoc.y).toBe(0);
  });
  ...
  test("test simple", () => {
    var wires = createWires("R8,U5,L5,D3");
    process(origin, wires);
    output(wires);
    expect(wires[0].positions.length).toBe(22);
    var finalLoc = wires[0].positions[wires[0].positions.length - 1];
    expect(finalLoc.x).toBe(3);
    expect(finalLoc.y).toBe(-2);
  });

  test("find intersects", () => {
    var wires = [{
        positions: [{
          x: 5,
          y: 6
    ...
    var intersects = findIntersects(origin, wires);
    expect(intersects.length).toBe(1);
  });

  test("intersects on example wires 1", () => {
    var wires = createWires("R8,U5,L5,D3", "U7,R6,D4,L4");
    process(origin, wires)
    var intersects = findIntersects(origin, wires);
    var distances = findDistances(origin, intersects);
    expect(distances.length).toBe(3);
    distances.sort((a, b) => a - b)
    expect(distances[1]).toBe(6);
  });

  test("intersects on example wires 2", () => {
    var wires = createWires("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83");
    process(origin, wires);
    var intersects = findIntersects(origin, wires);
    var distances = findDistances(origin, intersects);
    distances.sort((a, b) => a - b);
    expect(distances[1]).toBe(159);


    var shortestSteps = findShortestSteps(wires, intersects)
    expect(shortestSteps.length).toBe(5);
    shortestSteps.sort((a, b) => a - b)
    expect(shortestSteps[1]).toBe(610);
  });

  test("intersects on example wires 3", () => {
    var wires = createWires("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7");
    process(origin, wires);
    var intersects = findIntersects(origin, wires);
    var distances = findDistances(origin, intersects);
    distances.sort((a, b) => a - b);
    expect(distances[1]).toBe(135);

    var shortestSteps = findShortestSteps(wires, intersects)
    expect(shortestSteps.length).toBe(6);
    shortestSteps.sort((a, b) => a - b)
    expect(shortestSteps[1]).toBe(410);
  });
});


test("count on example wires 1", () => {
  var wires = createWires("R8,U5,L5,D3", "U7,R6,D4,L4");
  process(origin, wires)
  var intersects = findIntersects(origin, wires);
  var distances = findDistances(origin, intersects);
...
function output(wires) {
  ...
  for (wire of wires) {
    for (position of wire.positions) {
      output[position.y][position.x] = ++counter % 10
    }
  }
  output[origin.y][origin.x] = 'o'

  var text = []
  for (var y = 0; y < output.length; y++) {
    text.push(output[y].join(''))
  }
